...
const (
	...
	indexDatabaseAlias  = "ImqsServerSearch"
...
// ToDefaultParser extracts the config field properties to a default parser
// object
func (c *ConfigField) ToDefaultParser() *DefaultParser {
	return &DefaultParser{
		ExcludeEntireStringFromResult: c.ExcludeEntireStringFromResult,
		MinimumTokenLength:            c.MinimumTokenLength,
	}
}

// ConfigRelation is a relation to a table and is thought of as a child
// of a ConfigTable struct
...
// Equals assesses the equaliy of 2 config relations
...
type ConfigDatabase struct {
	...
	DBConfig *serviceconfig.DBConfig `json:"-"`
...
// drop connects to the postgres master database and drops the database
...
func (c *Config) getIndexDbAlias() string {
	if c.IndexDBAlias == "" {
		return indexDatabaseAlias
	}
	return c.IndexDBAlias
}

func (c *Config) getGenericDbAlias() string {
	if c.GenericDBAlias == "" {
		return genericDatabaseName
	}
	return c.GenericDBAlias
}
...
func (c *Config) dumpRelationshipGraph() string {
	s := ""
	for _, dbName := range c.databasesOrderedByName() {
...
func (c *Config) updateRowCounts(e *Engine) error {
	...
	for dbName, dbConfig := range c.Databases {
		...
		for tableName := range dbConfig.Tables {
			exists := false
